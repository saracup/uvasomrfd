...
class uvasomrfd_search_widget extends WP_Widget {
	function __construct() {
		parent::__construct(
			'uvasomrfd_search_widget', // Base ID
			'UVA SOM Faculty Search Widget', // Name
			array( 'description' => 'UVA SOM Faculty Search Widget' ) // Args
		);
	}
	// constructor
	//function uvasomrfd_search_widget() {
		//parent::WP_Widget(false, $name = __('UVA SOM Faculty Search Widget', 'uvasomrfd_search_widget') );
	//}
...
<p>
<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'uvasomrfd_search_widget'); ?></label>
...
<?php
	...
	function widget($args, $instance) {
	   ...
	   <form action="<?php bloginfo('url'); ?>" method="get">
		<?php
		if('on' == $instance['rd'] ) {
		custom_taxonomy_dropdown( 'research-discipline','Research Discipline' );
		}
		if('on' == $instance['primary'] ) {
		custom_taxonomy_dropdown( 'primary','Primary Department' );
		}
...
// register widget
add_action('widgets_init', create_function('', 'return register_widget("uvasomrfd_search_widget");'));
?>
